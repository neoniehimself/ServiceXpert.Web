...
public static class SxpEnumUtil
{
    ...
    public static Dictionary<int, string> ToDictionary<T>() where T : struct, Enum =>
        Enum.GetValues(typeof(T)).Cast<T>().ToDictionary(e => Convert.ToInt32(e), e => e.ToString());
    ...
    public static List<string> ToList<T>() where T : struct, Enum => Enum.GetNames(typeof(T)).ToList();
}
